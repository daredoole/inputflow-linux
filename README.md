...
1. `screen_width` and `screen_height` from config, or `--screen-width` and `--screen-height`
2. `MWB_SCREEN_WIDTH` and `MWB_SCREEN_HEIGHT`, if both are set
3. KDE logical geometry from `kscreen-doctor -o`, when available
...
Explicit overrides are still recommended on stacked or mixed-DPI multi-monitor desktops, and inside containers where automatic detection may not reflect the host desktop accurately.
...
Example with explicit `run` overrides:
